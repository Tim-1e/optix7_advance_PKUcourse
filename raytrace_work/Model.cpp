  ...
  Model *loadOBJ(const std::string &objFile, std::vector<LightParams>& lights)
  {
    ...
    for (int shapeID=0;shapeID<(int)shapes.size();shapeID++) {
      ...
      for (int materialID : materialIDs) {
        ...
        //std::cout << materials[materialID].shininess << std::endl;
        for (int faceID = 0; faceID < shape.mesh.material_ids.size(); faceID++) {
          ...
          mesh->diffuseTextureID = loadTexture(model,
                                               ...
                                               modelDir);
          mesh->spec = (const vec3f&)materials[materialID].specular;
          mesh->specTextureID = loadTexture(model,
            knownTextures,
            materials[materialID].specular_texname,
            modelDir);
          mesh->alpha_ = (const float&)materials[materialID].illum;
